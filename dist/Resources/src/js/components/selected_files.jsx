// @flow
/**
 * @file       Component shows the currently selected files. This component only
 *             renders in Filepicker mode.
 */
import React from 'react';
import R from 'ramda';
import { translate } from 'react-i18next';

type PropsType = {
    name: string,
    selectedFiles: FileType[],
    selectFile: (string) => void,
    showPreview: (imageUrl: null | string) => GenericActionType,
    t: (string) => string,
};

type DefaultPropsType = {};
type SelectedFilesStateType = {};

const SelectedFiles = (props: PropsType): null | React$Element<*> => {
    if (props.selectedFiles.length === 0) {
        return null;
    }

    let files = R.map((file: FileType): React$Element<*> => {
        let preview = <span className="fa fa-file" />;
        if (file.thumb) {
            const original = typeof file.original === 'undefined' ? null : file.original;
            const p = { onClick: () => { props.showPreview(original); } };
            preview = (<img
                src={file.thumb}
                // alt={file.name}
                alt={'no thumb'}
                {...p}
            />);
        }
        let p = {};
        if (typeof props.selectFile !== 'undefined') {
            p = { onClick: () => { props.selectFile(file.id); } };
        }
        return (<div key={file.id} className="btn btn-default" {...p}>
            {preview}
            <span className="name">{file.name}</span>
            <span className="remove">&times;</span>
            <input type="hidden" name={props.name} value={file.id} />
        </div>);
    }, props.selectedFiles);

    if (files.length === 0) {
        files = <span className="none-selected">{props.t('common.noFilesSelected')}</span>;
    }

    return (
        <div className="text-left file-picker-selection">
            {files}
        </div>
    );
}

export default translate('common')(SelectedFiles);
