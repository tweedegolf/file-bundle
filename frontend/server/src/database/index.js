...
const deleteFolder = (deletedFolderId: string): ReturnType => {
    const collectedItemIds: { fileIds: string[], folderIds: string[] } = {
        fileIds: [],
        folderIds: [],
    };
    ...
    R.uniq(collectedItemIds.fileIds).forEach((id: string) => {
        const file = filesById[id];
        filesById[id] = { ...file, isTrashed: true };
    });

    R.uniq(collectedItemIds.folderIds).forEach((id: string) => {
...
const moveItems = (
    ...
    }, [...fileIds, ...R.uniq(collectedItemIds.fileIds)]);
    ...
    }, [...folderIds, ...R.uniq(collectedItemIds.folderIds)]);
